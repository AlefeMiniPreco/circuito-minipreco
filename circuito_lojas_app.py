# -*- coding: utf-8 -*-
# circuito_lojas_app.py — VERSÃO COM FOCO NO TEMPO RESTANTE
...
/* Estilos do Pódio */
.podio-card h2 { font-size: 2em; margin: 8px 0 2px 0; color: #6EE7B7; } /* Cor destaque para o tempo */
...
def format_hours_and_minutes(hours_float: float):
    ...
    if hours_float < 0:
        return f"-{format_hours_and_minutes(abs(hours_float))}" # Formata tempo negativo (ultrapassou a meta)
...
def calculate_final_scores(df: pd.DataFrame, etapas_scores_cols: list, duracao_total_horas: float, baseline_horas: float):
    ...
    df_copy["Avanço_Horas"] = baseline_horas + (df_copy["Boost_Total_Min"] / 60.0)
    if duracao_total_horas > 0:
        df_copy["Progresso"] = (df_copy["Avanço_Horas"] / duracao_total_horas) * 100.0
    else:
        df_copy["Progresso"] = 0.0
    df_copy["Tempo_Faltante_Horas"] = duracao_total_horas - df_copy["Avanço_Horas"] # Não mais .clip(lower=0)
    df_copy["Rank"] = df_copy["Tempo_Faltante_Horas"].rank(method="dense", ascending=True).astype(int) # Rank crescente
    df_copy.sort_values(["Tempo_Faltante_Horas","Nome_Exibicao"], ascending=[True,True], inplace=True, ignore_index=True) # Sort crescente
    return df_copy
    ...
    for i, row in top3.iterrows():
        with cols[i]:
            st.markdown(
                ...
                f"<p class='secondary-metric'>Tempo Restante</p>"
...
def build_pista_fig(data: pd.DataFrame, duracao_total_horas: float) -> go.Figure:
    ...
    # Eixo X agora representa o TEMPO RESTANTE
    min_tempo_faltante = data['Tempo_Faltante_Horas'].min() if not data.empty else 0
    limite_eixo_inferior = min(0, min_tempo_faltante)
    
    # Desenha as pistas
    for i in range(len(data)):
        fig.add_shape(type="rect", x0=limite_eixo_inferior, y0=i-0.5, x1=duracao_total_horas, y1=i+0.5, line=dict(color='rgba(255, 255, 255, 0.1)', width=1.5), fillcolor="#2C3E50", layer="below")
        
    # Linha de Chegada (à esquerda, no marco zero)
    ...
    for i in range(math.ceil((len(data)+0.5) / square_size)):
        for j in range(num_cols):
            ...
            fig.add_shape(type="rect", x0=0 - ((j+1) * square_size), y0=i*square_size - 0.5, x1=0 - (j * square_size), y1=(i+1)*square_size - 0.5, line=dict(width=0.5, color="black"), fillcolor=color, layer="above")

    # Adiciona os carros e textos
    for i, row in data.iterrows():
        hover_text = (f"<b>{row['Nome_Exibicao']}</b><br>Tempo Restante: {format_hours_and_minutes(row['Tempo_Faltante_Horas'])}<br>Progresso: {row['Progresso']:.1f}%<br>Avanço Total: {format_hours_and_minutes(row['Avanço_Horas'])}<br>Rank: #{row['Rank']}")
        # Posição do carro é o tempo restante
        posicao_x = row['Tempo_Faltante_Horas']
        # Adiciona o carro
        fig.add_layout_image(dict(source=CAR_ICON_URL, xref="x", yref="y", x=posicao_x, y=i, sizex=max(1.8, duracao_total_horas / 20), sizey=0.9, layer="above", xanchor="center", yanchor="middle"))
        # Adiciona o nome da loja
        fig.add_trace(go.Scatter(x=[posicao_x], y=[i-0.4], mode='text', text=row['Nome_Exibicao'], textfont=dict(color='white', size=10), showlegend=False))
        # Adiciona alvo de hover invisível
        fig.add_trace(go.Scatter(x=[posicao_x], y=[i], mode='markers', marker=dict(color='rgba(0,0,0,0)', size=35), hoverinfo='text', hovertext=hover_text, showlegend=False))

    fig.update_xaxes(range=[limite_eixo_inferior - 1, duracao_total_horas + 1], title_text="← Tempo Restante para a Chegada (horas)", fixedrange=True)
...
def render_geral_page():
    ...
    headers = ["Rank", "Loja", "Tempo Restante", "Progresso", "Avanço Total"]
    if show_details:
    ...
    for i, row in df_final.iterrows():
        ...
        html.append(f"<td>{format_hours_and_minutes(row['Avanço_Horas'])}</td>")
        if show_details:
