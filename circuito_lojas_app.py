...
def render_podio_table(df_final: pd.DataFrame):
    ...
    html_table.append("<thead><tr><th style='width:80px;'>#</th><th>Loja</th><th style='width:120px; text-align:center;'>Avanço (min)</th><th style='width:140px; text-align:center;'>Progresso</th><th style='width:220px; text-align:center;'>Premiação</th></tr></thead><tbody>")

    for _, row in winners.iterrows():
        pos, nome, avanco, progresso = int(row["Rank"]), row["Nome_Exibicao"], row["Avanço_Total"], row["Progresso"]
        if pos == 1:
            premio, premio_class = PREMIO_TOP1, "gold"
        elif pos in (2, 3):
            premio, premio_class = PREMIO_TOP3, "silver"
        elif pos in (4, 5):
            premio, premio_class = PREMIO_TOP5, "bronze"
        else:
            premio, premio_class = PREMIO_DEMAIS, "other"
